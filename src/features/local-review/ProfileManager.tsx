...
export function ProfileManager({
	...
	const defaultCount = profiles.filter(
		(profile) => profile.enabledByDefault,
	).length;
			...
			<div className="flex flex-col gap-4 border-b border-border p-4 lg:flex-row lg:items-start lg:justify-between">
				<div>
					...
					<p className="mt-1 text-sm text-muted-foreground">
						Profiles are application-owned and scoped outside reviewed
						repositories.
					</p>
				</div>
					...
					<Dialog open={open} onOpenChange={setOpen}>
						...
						<DialogContent className="sm:max-w-2xl">
							<DialogHeader>
								...
								<DialogDescription>
									The profile is stored by the application and scoped to where
									it should apply.
								</DialogDescription>
							</DialogHeader>
